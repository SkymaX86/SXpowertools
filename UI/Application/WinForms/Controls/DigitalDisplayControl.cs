...
namespace SXpowertools.UI.Application.WinForms.Controls
{
	...
	public partial class DigitalDisplayControl : UserControl
	{
        ...
        private DigitTextAlign _digitTextAlign = DigitTextAlign.Center;
        [Browsable(true), DefaultValue(DigitTextAlign.Center)]
        public DigitTextAlign DigitTextAlign
        {
            get { return _digitTextAlign; }
            set { _digitTextAlign = value; Invalidate(); }
        }
		...
		private void DigitalGauge_Paint(object sender, PaintEventArgs e)
		{
			...
			{
                switch (_digitTextAlign)
                {
                    case DigitTextAlign.Right:
	...
	public class SevenSegmentHelper
	{
		...
		public void DrawDigits(string text, Font font, Brush brush, Brush brushLight, float x, float y)
		{
			for (int cnt = 0; cnt < text.Length; cnt++)
			{
				...
				// For dot .
                    ...
                    x = DrawSemiColon(font, brush, x, y);
			}
		}
        ...
        private float DrawSemiColon(Font font, Brush brush, float x, float y)
        {
            Point[][] dotPoints = new Point[1][];

            dotPoints[0] = new Point[] {new Point( 3, 61), new Point( 6, 55), new Point( 9, 61), new Point( 6, 75)};
